...
export function buildAgyBootstrapEnvelope(input: {
    ...
    timestampMs?: number;
    order?: 'task-first' | 'context-first';
    ...
    const order = input.order === 'context-first' ? 'context-first' : 'task-first';
    ...
    const orderedSections = order === 'context-first'
        ? [operationalSection, historySection, taskSection]
        : [taskSection, operationalSection, historySection];
    const sections = [bootstrapSection, ...orderedSections.filter(Boolean)];
