...
async function isMainAgentBusy(): Promise<boolean> {
    try {
        const mod = await import('../agent/spawn.js');
        return mod.isAgentBusy();
    } catch (err) {
        console.warn('[heartbeat] busy check failed:', (err as Error).message);
        return false;
    }
}
...
async function runHeartbeatJob(job: Record<string, any>) {
    ...
    if (await isMainAgentBusy()) {
        const queued = queueHeartbeatJob(job, 'agent_busy', 'defer');
        console.log(`[heartbeat:${job["name"]}] ${queued ? 'deferred' : 'already deferred'} while agent is busy (${pendingJobs.length} pending)`);
        return;
    }
    heartbeatBusy = true;
    try {
...
export async function drainPending() {
    ...
    if (await isMainAgentBusy()) return;
